...
if len(sys.argv) != 2:
    sys.exit("Usage: python recognition.py model.keras")
...
while True:
    ...
    for i in range(ROWS):
        row = []
        # Draws grid; darkens written cells; fills neighbors on click
        for j in range(COLS):
            rect = pygame.Rect(
                OFFSET + j * CELL_SIZE,
                OFFSET + i * CELL_SIZE,
            ...
            # If a cell has been written on, darken the cell
            if handwriting[i][j]:
            ...
            if mouse and rect.collidepoint(mouse):
                ...
                # Fills diagonal neighbor cell with lower intensity
